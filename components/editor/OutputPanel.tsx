...
export default function OutputPanel({ toolId, error, outputLanguage = 'json', showOutputOnlyHint = false }: OutputPanelProps) {
  ...
  const searchResult = useMemo(
    ...
    [output, searchCase, searchQuery, searchRegex, searchWholeWord]
  );
  ...
  const applySelection = useCallback(
    (index: number) => {
      ...
      try {
        ...
        // Ignore transient model/line race conditions while content refreshes.
      }
    },
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (mod && event.key.toLowerCase() === 'f' && (editorFocused || withinPanel)) {
      ...
      if (event.key === 'F3' && (editorFocused || searchOpen)) {
        event.preventDefault();
        if (event.shiftKey) goPrev();
        else goNext();
      }
  ...
  }, [editorFocused, focusSearchInput, goNext, goPrev, searchOpen]);
        ...
        {!error && !treeView && (
          <>
            <button
              ...
              className={`${panelBtnClass} ${searchOpen ? 'bg-dt-soft text-dt-text' : ''}`}
              title="Search"
            >
              Search
            </button>
